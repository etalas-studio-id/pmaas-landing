...
export const Services: React.FC = () => {
  ...
  // Performance: Throttle mouse events to frame rate using requestAnimationFrame
  // This prevents layout thrashing when moving mouse quickly over cards
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((service, index) => (
          <FadeIn key={index} delay={index * 100} className="h-full" scale>
            ...
            >
              {/* Spotlight Effect - GPU Accelerated Layer */}
              <div 
                className="pointer-events-none absolute -inset-px opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-0 will-change-[opacity]"
                style={{
